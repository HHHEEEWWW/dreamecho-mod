...
// 在 interop/Assembly-CSharp.dll 中按关键词搜索类型
var interopDir = @"E:\steam\steamapps\common\DreamEcho\BepInEx\interop";
var coreDir = @"E:\steam\steamapps\common\DreamEcho\BepInEx\core";
var keywords = args.Length > 0 ? args[0].Split(',') : new[] { "affix" };
var showMethods = args.Contains("--methods");
var showFields = args.Contains("--fields");
var full = args.Contains("--full");

var paths = Directory.GetFiles(interopDir, "*.dll").Concat(Directory.GetFiles(coreDir, "*.dll"))
...
{
    Console.WriteLine($"\n===== keyword: {kw} =====");
    var hits = types.Where(t => t.FullName != null && t.FullName.IndexOf(kw, StringComparison.OrdinalIgnoreCase) >= 0)
